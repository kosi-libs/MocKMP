...
[NOTE]
====
A placeholder is never meant to be used as a real value — it only has to exist so a call like `isAny<T>()` can
typecheck. For most types the processor builds one automatically (an empty collection, an instance built from an
accessible constructor, an implementation whose members all throw...). When it genuinely cannot — for example a
type with no public constructor, reachable only as a parameter of a mocked interface's member — the generated
placeholder function throws a `MocKMPNoPlaceholderException` explaining why, instead of silently producing
something broken.
