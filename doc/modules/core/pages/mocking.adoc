...
CAUTION: Only *interfaces* and *abstract classes* can be mocked!
A mock is a generated implementation of its type, so a type that cannot be implemented (a concrete class, an object, an enum) cannot be mocked.
This is by design and is not likely to change.


== Requesting generation

You can declare that a class or a function needs a specific mocked type by using the `@UsesMocks` annotation.
...
NOTE: `mocker.mock<T>()` only works for a type requested directly — with `@UsesMocks`, or a `@Mock`
property. Unlike xref:faking.adoc[faking], where a type can be generated as a side effect of faking
another one (and is deliberately not reachable through `fake<T>()` for that reason), nothing is ever
mocked as a side effect of mocking something else, so there is no such case to worry about here.


[#abstract-classes]
== Mocking abstract classes

An abstract class is mocked exactly like an interface, save for two differences:

* Only its *abstract* members are mocked.
  A concrete member is left alone: it runs its real implementation, and can neither be given a behaviour with `every` nor be verified.
* The generated mock *extends* it, so its constructor runs, with an inert value for each of its
  parameters, resolved the same way xref:faking.adoc[faking] resolves one.

[source,kotlin]
----
abstract class Repository(val name: String) {
    abstract fun count(): Int
    fun describe(): String = "$name repository" //<1>
}

@UsesMocks(Repository::class)
class MyTests {
    @Test fun test() {
        val mocker = Mocker()
        val repository = mocker.mock<Repository>()

        mocker.every { repository.count() } returns 42
        assertEquals(" repository", repository.describe()) //<2>
    }
}
----
<1> Not abstract, so it is not mocked: it runs, over the constructor arguments the mock was built with.
<2> `name` is a faked `String`, so it is empty.

NOTE: This is the reverse of an interface's default method, which *is* overridden and mocked, and therefore must be given a behaviour like any other function.
...
[CAUTION]
====
A `Mocker`, and every mock created from it, must be used from a *single thread*.
Calls arriving concurrently corrupt the mocker's state instead of failing cleanly: calls go missing or land out of order, and a later `verify` fails for reasons unrelated to the code under test.

If the code under test dispatches, inject the dispatcher it uses so that the mocked calls still run on the test thread, rather than letting it reach a real background one.
====
...
class MyTests {
    @Test fun myUnitTest() {
        ...
        mocker.backProperty(place, Place::name, default = "")
...
`isEqual`, `isNotEqual`, `isSame` and `isNotSame` return the value you pass them, so they never need a
placeholder for the argument type. `isAny`, `isNull`, `isNotNull` and `isInstanceOf` have no such
value to return, so they hand back a _placeholder_ instead — see the note below.

[WARNING]
====
`isInstanceOf` takes the `KClass` to check against as a regular argument, not as a type parameter:
...
Do *not* write `isInstanceOf<AdminCallback>()`. `isInstanceOf` is still a `reified` function, but that reified
type parameter is only there so MocKMP can produce a placeholder return value for `every`/`verify` — it must stay
inferred from the surrounding call (here, the declared type of `registerCallback`'s parameter), never given
explicitly. Passing an explicit type argument forces the placeholder lookup to happen for that exact (possibly
narrower) type instead, which the generated code has no placeholder for unless that precise type is used
elsewhere in your mocked interfaces — and mocking fails at runtime with a "Could not find a way to get a
reference of ..." error that names this exact mistake as the likely cause.
====

[NOTE]
====
A placeholder is never meant to be used as a real value — it only has to exist so a placeholder-returning
constraint like `isAny<T>()` can typecheck. For most types the processor builds one automatically (an empty
collection, an instance built from an accessible constructor, an implementation whose members all throw...). When
it genuinely cannot — for example a type with no public constructor, reachable only as a parameter of a mocked
interface's member — the generated placeholder function throws a `MocKMPNoPlaceholderException` explaining why,
instead of silently producing something broken. (`isEqual` and the other value-carrying constraints above sidestep
this entirely — they return the value you gave them.)

Every value a generated placeholder builds is first looked up through the mocker's references, so
`mocker.useReference(instance)` fixes it whether the unbuildable type is the one `isAny<T>()` asks for directly
*or* one buried inside a placeholder the processor could otherwise build — registering the `Amount` in
`isAny<MoneyTransfer>()` where `MoneyTransfer(val amount: Amount)` and `Amount` has no public constructor.
====
...
mocker.every { api.getUserById(isAny()) } runs { args -> //<1>
    val idArg = args[0] as Int
    if (idArg == 42) fakeUser
...
----
<1> The block is an ordinary lambda, not an inline one: its *last expression* is the returned value.
A `return` here would try to return from the enclosing function and will not compile.
...
[source,kotlin]
----
val callback: (User) -> Unit = mockFunction1(mocker) //<1>
...
----
<1> The `mocker` is mandatory: it is what records the call, exactly as it does for a mocked interface.
The declared type of the property is what tells `mockFunction1` the argument and return types, so there is nothing else to pass.
...
[source,kotlin]
----
val callback: (User) -> Unit = mockFunction1(mocker) {} // implicit Unit
----

You can mock suspending functions with `mockSuspendFunctionX`, which works the same way.
Note that a suspending function mock must be defined with `everySuspending` and checked with `verifyWithSuspend`, just like a suspending method:

[source,kotlin]
----
val onUserLoaded: suspend (User) -> Unit = mockSuspendFunction1(mocker) //<1>
mocker.everySuspending { onUserLoaded(isAny()) } returns Unit

userRepository.observeUser(onUserLoaded)

mocker.verifyWithSuspend { onUserLoaded(fakeUser) }
----
<1> Creating the mock is *not* itself a suspending operation, so this line does not need to be in a coroutine.
...
Like `isAny`, single-argument `isValid` resolves a placeholder for the argument type. If that type has
no placeholder, pass a real stand-in value as `placeholder` — it is never checked, only the
constraint is:

[source,kotlin]
----
fun ArgConstraintsBuilder.isTournamentNamed(name: String): Tournament =
    isValid(
        ArgConstraint(description = { "isTournamentNamed($name)" }) {
            if (it.name == name) ArgConstraint.Result.Success
            else ArgConstraint.Result.Failure { "Expected tournament <$name>, got <${it.name}>" }
        },
        placeholder = Tournament.of(name),
    )
----
