...
public class MocKMPProcessor(
    ...
    private fun KSType.toFunName(): String {
        val prefix = buildString {
            if (isMarkedNullable) append("Nul")
        ...
        return (prefix +
                ...
                else "${declaration.simpleName.asString()}X${arguments.joinToString("_") {
    ...
    private fun privateProcess(resolver: Resolver): List<KSAnnotated> {
        ...
        val mocks = HashMap<KSClassDeclaration, ClassName>()
        val toFake = HashMap<KSType, ToProcess>()
        val fakes = HashMap<KSType, MemberName>()
                    ...
                    if (!param.hasDefault) {
                        ...
                        if (paramType.nullability == Nullability.NOT_NULL) {
                            ...
                            if (paramType.declaration is KSTypeParameter) {
                                val index = cls.typeParameters.indexOf(paramType.declaration)
                                paramTypeRef = type.arguments.getOrNull(index)?.type
                                    ?: error(param, "Could not resolve generic parameter $param (${process.references.firstOrNull()?.location})")
                                paramType = paramTypeRef.resolve()
                            }
                            val paramTypeName = paramTypeRef.toRealTypeName(cls.typeParameters.toTypeParameterResolver())
                            if (paramTypeName.qualified() !in builtins && paramType !in providedFakes && paramType !in toFake) {
                                addFake(paramType, process.files, param)
                                toExplore.add(paramType to process)
                            }
                        }
                    }
                }
        ...
        if (mocks.isNotEmpty() || fakes.isNotEmpty()) {
            run {
                ...
                val gFun = FunSpec.builder("mock")
                    ...
                    .returns(T)
                if (multiplatform) {
                    gFun.addModifiers(KModifier.ACTUAL)
                }
                    ...
                    mocks.forEach { (itf, mock) ->
                        val mockTypeName: TypeName =
                            if (itf.typeParameters.isNotEmpty()) {
                                mock.parameterizedBy(
                                    *itf.typeParameters.map {
                                        it.bounds.first().resolve().toTypeName()
                                    }.toTypedArray()
                                )
                            } else {
                                mock
            ...
            run {
                ...
                if (fakes.isNotEmpty()) {
                    gFun.beginControlFlow("return when (type)")
                    fakes.forEach { (type, fake) ->
                        gFun.addStatement("type_${type.toFunName()} -> %M() as T", fake)
                    }
            ...
            run {
                ...
                if (toInject.isNotEmpty()) {
                    ...
                    toInject.forEach { (cls, _) ->
                        gFun.addStatement("is %T -> %M(receiver)", cls.toClassName(), MemberName(cls.packageName.asString(), "injectMocks"))
                    }
